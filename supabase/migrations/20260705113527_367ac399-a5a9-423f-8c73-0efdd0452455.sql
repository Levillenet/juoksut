...
BEGIN
  ...
  -- Poista ikäluokka-etuliite (M15, N15, P15, T15, M jne.)
  s := regexp_replace(s, '^\s*[mntp][0-9]*\s+', '');
  -- Normalisoi väli numeron ja "m"-yksikön välissä (100m -> 100 m)
  s := regexp_replace(s, '([0-9])[[:space:]]*m([[:space:]]|$)', '\1 m\2', 'g');
  -- Yhtenäistä aitatermi
  s := regexp_replace(s, '\maidat\M', 'aj', 'g');
  -- Kävelyn lyhenne
  s := regexp_replace(s, 'käv\.', 'kävely', 'g');
  -- Poista tuplavälit
