...
CREATE OR REPLACE FUNCTION public.get_shared_watch_history(p_token text, p_exclude_competition_id integer)
 RETURNS TABLE(athlete_key text, event_name text, event_category text, sub_category text, result_text text, result_numeric double precision, competition_date timestamp with time zone, age_class text)
 LANGUAGE sql
